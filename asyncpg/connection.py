...
class Connection(metaclass=ConnectionMeta):
    ...
    def __init__(
        ...
        self._recent_statements = collections.deque(maxlen=20)
    ...
    def _maybe_gc_stmt(self, stmt):
        if stmt.refs == 0 and not self._stmt_cache.has(
    ...
    async def _cancel(self, waiter):
        try:
            ...
            self._cancellations.discard(compat.current_asyncio_task(self._loop))
            if not waiter.done():
